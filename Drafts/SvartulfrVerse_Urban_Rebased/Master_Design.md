...
`world_mode`: sandbox (validated — seed Section 1 field `World Mode: sandbox`)
`intimacy_in_scope`: true (seed Section 8 contains active intimacy material; Kaladin routed to PRINCIPAL intimacy profile)
`current_phase`: 2
`status`: IN_PROGRESS
`round`: 0
...
| Phase | Status | Notes |
|---|---|---|
| 0 Interviewer | COMPLETE | Seed-revision interview 2026-07-10 |
| 1 Refiner | COMPLETE | This document |
| 2 Architect | PENDING | |
| 2.5 Intimacy Architect | PENDING | Intimacy in scope — Kaladin (principal), Mac/Fade (roster) |
| 3 Editor | PENDING | |
| 3.5 Voice Auditor | PENDING | |
| 3.6 Arc Transition Auditor | SKIPPED | Sandbox mode — no arc seams exist |
| 3.7 Intimacy Auditor | PENDING | Intimacy in scope |
| 4 Compiler | PENDING | |
| 5 Prompt Engineer | PENDING | |
| 5.5 Preset Resync | PENDING | |
...
- **Plural-species note:** Solarton/Blackwood are home to many supernatural species beyond lupines — orcs, demi-humans, vampires, ghoul, fae, dryads, sirens, trolls, etc. LSE morphology (Rule 1) governs werewolves; other species follow their own natures. Orcs/demi-humans cannot mimic a human form.
...
- "{{user}} / arc trajectory" (in arc mode only — sandbox has no arc trajectory)
...
- **Standing Goal (active):** Maintain suffocating control over {{user}}'s environment — micromanage college schedule, deploy drones, interrogate any suitor, all oblivious to {{user}}'s actual secret life. At the Family Wanted Level's peak (§5B.6) escalates to a full DCC "extraction" of {{user}} — played as farce, never real danger.
...
- **Sample lines:** Sierra — "Okay but actually, your brother is *outside*." Scarlett — "Perfect. Let's lie. Obviously."
...
**NPC intimacy routing (§8, intimacy in scope):** Principal w/ sexual presence → Kaladin → full Intimacy Profile. Roster w/ sexual presence → Mac, Fade → §6.5 compact intimate stat blocks. Roland/Sierra/Scarlett/Vito/Logan/Wulfnic/Edric/Marcus/Ut/Zefir → no sexual presence (Edric = child hard-rule). Sandbox intimacy posture is a *standing* function (§8 of seed), not per-arc.
