...
[User intent — start a new world]
 ...
 Recommends corrections for any conflicts found — manual user application required.
      ...
      v
 PHASE 6: THE JANITOR BUILDER
 Generates JanitorAI ready bot profiles, bios, and Lorebook scripts by combining 
 exported JSON files with custom HTML/MD templates.
      v
 ✅ PIPELINE COMPLETE
```
...
| 6 Janitor Builder | Profile/Bio uses standard World Name. | Same format, Profile/Bio uses Sandbox parameters. |
...
| 3.6 Arc Transition   | COMPLETE | 0  | ARC TRANSITION AUDITOR SIGN-OFF (SKIPPED in sandbox mode) |
...
**Invoke:** `@agent_roles/02b_The_Intimacy_Architect.md`
...
## PHASE 6: EXTERNAL EXPORTS — THE JANITOR BUILDER

**Invoke:** `@agent_roles/06_The_Janitor_Builder.md`
**Input:** `Export/` JSONs + `ChatPreset.json`
**Output:** `Export/[WorldName]_JanitorAI_Script_*.js`, `Export/Janitor_Bio_[CharName].html`, and `Export/[CharName]_JanitorAI.md`

**Mandatory end-of-pipeline phase:** The agent runs three python scripts (`build_janitor.py`, `build_bio.py`, `build_janitor_profile.py`) using the approved templates to generate storefront bios, modular JS scripts, and complete bot profiles. This isolates the generation of platform-specific texts from the pure JSON compilation logic.
