...
2. The world — sensory signature, rules with costs, factions, locations, species, concepts
3. The protagonist ({{user}}) — wound, hidden layer, contradiction, power and limits, arc trajectory, physical, voice
4. Characters — central wound, shield, crack, voice with sample lines, physical in anatomical order, relationships, NPCs with sample lines
5. Arcs — hidden information rules, dramatic beats, NPC shifts, entry and exit triggers, tone and pacing
6. Technical specifications — cards, lorebooks, depth_prompt assessment per character
7. Test scenarios (Section 7b) — three to five specific roleplay moments the user intends to play through
8. **Intimacy & Sexuality (Section 8)** — world-level posture toward sex, per-character intimacy substrate, per-arc thematic functions and scene types, tonal hard rules for intimate content. Conditional on the world containing intimate scenes.
