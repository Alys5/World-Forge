...
- **The always-regenerate rules invert.** Every "regenerate (always)" disposition derives from the protagonist or arc spine changing; in rebaseline neither changes, so Section 3 (protagonist), Section 5 (arcs / Sandbox Charter), Section 7b (test scenarios), per-arc/standing intimate functions, per-card style overrides, `Tier1_Random_Events.md`, `Tier1_Scene_Orchestrator.md`, and relationship-to-`{{user}}` content all **keep** at their post-revision state. The four Section 4 strip rules (Standing Goal / drift / belief / trauma trajectory) invert to carry.
