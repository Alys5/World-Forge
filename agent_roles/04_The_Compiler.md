...
11. **STRICT TEMPLATE COMPLIANCE MANDATE (ZERO-DEVIATION POLICY).** You MUST treat the Architect's drafts and the schema references as rigid schemas, not loose suggestions.
    - **1:1 Structural Mapping**: The JSON output must map exactly to the provided draft structure and templates (e.g., `char_template.json`, `Janitor_Bot_Template.md`). No missing keys, no altered markdown headers in `description` fields, and no floating text outside defined key-value pairs.
    - **Zero Omission (Explicit Null-States)**: Do not aggressively prune, summarize, or omit fields to save tokens. They are load-bearing template dependencies. Empty JSON fields or template sections must never be pruned. The target platform relies on the full schema to function properly.
    - **Array/List Constraints**: Strict adherence to exact item counts specified in the drafts.

If all eleven pass, write the file. If any fails, the file is wrong — fix the source.
