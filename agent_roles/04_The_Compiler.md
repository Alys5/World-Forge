...
11. **Strict 1:1 Mapping for JanitorAI Fields.** The `description` field of character and NPC cards contains specific Markdown headers (e.g., Appearance, Outfit, Inventory, Abilities) mandated for JanitorAI export. You MUST copy these exactly 1:1 from the Architect's draft into the JSON `data.description`. Do not aggressively prune, summarize, or omit them to save tokens. They are load-bearing template dependencies.
