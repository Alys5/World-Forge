...
- Physical description structured for the Architect: face & lips → hair → eyes → body (chest, waist, hips, legs) → intimate areas (if applicable) → movement & posture → sensory signature → dietary preference. MUST include all foundational JanitorAI fields (Appearance, Outfit, Inventory, Abilities/Species, Sexuality, Speech Info) with explicit null-states if empty.
