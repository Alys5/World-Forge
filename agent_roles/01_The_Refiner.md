...
**Pass 1 — Validate the world default.** Confirm Section 1.5a has values for all six fields. If a field reads `DEFAULTS` or is empty, fill in the legacy convention (`third_omniscient` / `present` / `asterisks_for_thoughts_only` / `double_quotes` / `double_asterisks` / `standard`), ensure the `style_notes` include the mandatory constraints (First-person present tense for {{user}}, NO em-dashes, In-Universe Text in backticks, ***Narrator/Events*** in triple asterisks, Native language dialogue as `"phrase" ([your_language] translation)`), and note in your output that the default was applied. Confirm the values are valid enum members. If a value is not a valid enum member (e.g., user wrote "third person" instead of `third_omniscient`), normalize it to the closest enum and note the normalization. If normalization is ambiguous, log to `UNRESOLVED_QUESTIONS.md`.
...
- **Physical description** structured in the same anatomical order as character descriptions: face & lips → hair & beard → eyes → body (build, chest, shoulders) → movement & posture → sensory signature (smell, voice, presence) → dietary preference. This feeds the Protagonist Lorebook.
...
- Physical description structured for the Architect: face & lips → hair → eyes → body (chest, waist, hips, legs) → intimate areas (if applicable) → movement & posture → sensory signature → dietary preference (including `SPECIES: DIET` rules if supernatural). MUST include all foundational JanitorAI fields (Appearance, Outfit, Inventory, Abilities/Species, Sexuality, Speech Info) with explicit null-states if empty.
