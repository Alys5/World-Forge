...
**The per-card override question.** Ask separately, after the world default is locked: "Does this world have a Director/Narrator card sitting alongside companion cards? Or any card that is structurally incompatible with the world default — for example, a confessional companion card in an otherwise third-person world?" If yes, flag those cards now; the override is declared in Section 4 inside that card's entry, not here. The pipeline supports per-card overrides on `perspective` and `narration_marker` only. Tense, dialogue marker, emphasis marker, and paragraph register are world-coherence settings and cannot be per-card.
...
**Then card style override (only if flagged in Section 1.5).** If you flagged this card in the Section 1.5 per-card override question, capture the override now: "What perspective does this card need that's different from the world default? What narration marker does this card need? In one sentence, why is the world default structurally wrong for this card — not just less preferred?" Record the perspective override, the narration marker override (or `INHERIT` if only one of the two is overridden), and the rationale. If you didn't flag this card in Section 1.5, leave the override fields as `INHERIT` and skip this question. Do not introduce overrides reactively per character — the decision was made world-wide at Section 1.5.
