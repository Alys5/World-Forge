...
**New `JanitorAI_Profile_[NewName].md` (or Group Profile):** **TOKEN ECONOMY & BEHAVIORAL ENGINEERING MANDATE:** You are a strict behavioral engineer. Group Profiles must follow the **U-Shaped Prompt Curve Alignment**: Core Family (Tier 1) at TOP, Main NPC Roster (Tier 2) in MIDDLE, Shared Scenario/Global Dynamics at BOTTOM, and Rank Instinct Macros/Trigger Matrix/Formatting/Dialogue Samples at VERY BOTTOM. **Permanent Rent Hard-Cap:** Max 1,800 tokens total. **Signal-to-Noise Filter:** Zero narrative backstories or static lore; every bullet point must actively change behavior. **Symmetric Core Blocks:** `APPEARANCE`, `PSYCHOLOGICAL_PROFILE`, `SOCIAL_BEHAVIOR`, `SENSORY` with max 350 tokens per char. **NPC Roster:** Compress into tight inline behavioral records (e.g. `[MAIN_NPC: Name | Rank | Role | Trigger]`). Enforce Strict AnyPOV and strict turn-taking controls (max 1-2 speaking characters).
