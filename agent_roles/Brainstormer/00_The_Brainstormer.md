...
2a. **Extract the prose as a style sample — this is the one axis the document hands you complete.** Unlike a fresh-start brainstorm, where the Style Contract is reverse-engineered from touchstones and adjectives, here the user has given you a *finished sample of the exact prose they want*. Read it for the mechanics the Style Contract (downstream Section 1.5) is built from and record them as raw material attributed to the source: **perspective** (1st / 2nd / 3rd, and whose head you're in), **tense** (past / present), **paragraph register and rhythm** (terse vs. dense, dialogue-heavy vs. interior, lyrical vs. plain), and any distinctive markers (how dialogue, emphasis, and internal thought are rendered). You are *capturing a sample, not locking a contract* — the Interviewer confirms and the Refiner formalizes Section 1.5 — but "replicate this document's prose" is a first-class reason a user adapts, so capture it precisely rather than collapsing it to a vague "touchstone."
...
- [Note: extracted from the source as a sample; the Interviewer confirms and the Refiner formalizes Section 1.5.]
