...
- **In a `--then-brainstorm` chain** (a `/worldforge convert --rebaseline --then-brainstorm` run): the target project's `World_Seed.md` — read it completely, including the Conversion Manifest at the top. This is the consolidated world you are brainstorming *improvements* to.
