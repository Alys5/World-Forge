...
**The bright line:** any revision that requires changes to Master Design Section 1 (Core Concept & Tone) or Section 11a (Style Contract world defaults) is **out of scope for the revision pipeline**. The Reviser (Phase R0) classifies and bounces these to a full pipeline re-run via `/worldforge skip phase0`, reusing the existing `World_Seed.md`. Per-card overrides in Section 11b stay in revision scope; only world-default changes bounce. **`World Mode` (arc | sandbox) lives in Section 1, so flipping it is a bright-line bounce** — the revise pipeline edits a world *within* its mode; converting arc↔sandbox is a full rebuild.
