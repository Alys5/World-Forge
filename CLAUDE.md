...
- **Two postures.** The default (spec Sections 1–7) is the fresh-fragment start. The **improvement posture** (spec Section 8) brainstorms *changes to an already-consolidated world*: it reads the world's `World_Seed.md` + Conversion Manifest and diverges on what to add, rework, or deepen — respecting the world's spine (an idea that flips World Mode, swaps the protagonist, or overturns the core concept is flagged as reframe-conversion territory, not a rebaseline improvement). This is what the Converter's **`--then-brainstorm`** flag invokes (#10): after a rebaseline consolidates a revised world, the Brainstormer explores improvements, writes its notes, and the chain continues into the Interviewer's seed-revision posture, which reads those notes as *proposals*.

**Boundaries:** the Brainstormer is upstream of the pipeline, not a phase of it — it does not advance the Pipeline State Ledger, invoke downstream agents, or produce any first-class pipeline artifact. It is a front porch. It touches no other architectural principle.
...
- **Purely additive, never mandatory.** The manifest changes no existing entry and emits no new file. A world without it still works — the extension degrades to prose parsing. (Note: our comment convention is `[CharName] — [Aspect]` for major characters and `NPC — [Name]` for roster NPCs, so the extension's `^NPC —` prose fallback misses major characters; the manifest is what makes every character discoverable.)
- **The Compiler is the only writer** (principle #3 is unchanged — manifest emission is part of normal Export JSON authoring). Per-file manifests carry **file-local** uids; the combined `Group_Lorebook.json` gets a single **re-derived** manifest whose uids track the re-sequenced Group UIDs (the per-file manifests are dropped from the combined set). The revise pipeline's mini-Compiler regenerates a touched lorebook's manifest against preserved + new UIDs, keeping slug ids stable. `tools/validate_export.py` is the deterministic backstop (carrier flags, JSON-parse, slug validity, uid resolution).
- **Boundaries.** The manifest does not touch SillyTavern itself, the override architecture (#2), audit/apply separation (#3), Position Rationale (#4), or the preset. The contract also defines an *optional* per-message "turn tag" runtime lever (`<!--npcmem:{...}-->`); that is **not yet implemented** on the producer side and is a separate, preset/card-level change if pursued.
...
| `agent_roles/Brainstormer/00_The_Brainstormer.md` (optional ideation, #11) | `agent_roles/00_The_Interviewer.md` (Context Manifest "Load if present" + Section 4 interview-open note read `Brainstorm_Notes.md` as starting material; Section 9 reads improvement-posture notes as *proposals*), `agent_roles/Converter/00_The_Converter.md` (Section 9 Step H `--then-brainstorm` dispatches the Brainstormer's improvement posture ahead of the seed-revision Interviewer), `workflows/world-forge.md` + `workflows/world-forge-convert.md` (BRAINSTORM section, trigger rows, `--then-brainstorm` chain), `.kilo/kilo.jsonc` (`WorldForge-Brainstormer` seat) — the Brainstormer's two postures and the `--then-brainstorm` chain must read consistently across the spec, the Interviewer that consumes its notes, the Converter that invokes it, the orchestrators, and the Kilo config |
...
- **Keying the NPC Memory Manifest by UID instead of by stable slug, or copying a per-file manifest's uids into the Group lorebook.** Memory persistence depends on slug ids derived from the canonical name (principle #12); UIDs renumber on re-export. And the Group lorebook re-sequences all UIDs, so its combined manifest must be re-derived (Compiler Step 7.7i), never copied from the per-file manifests.
