...
- **Permanent vs Situational split:** 
  ...
  - **Q&A and SECRETS:** Queste sezioni devono essere RIGOROSAMENTE rimosse dal Bot Profile principale per risparmiare token e migrate nello script JS, che si occuperà di farle emergere dinamicamente nel contesto (sfruttando `context.character.personality`).
...
- **Initial Messages & Alternate Greetings:** Tutti i messaggi iniziali (inclusi gli scenari di gruppo) devono essere scritti in `Drafts/[WorldName]/Initial_Messages.md`. In fase di compilazione, gli script (`build_janitor.py` e `compile_cards.py`) estrarranno da questo file canonico per popolare gli slot multipli di Janitor e gli alternate greetings di ST.
