...
def build_janitor(world_name):
    ...
    for filename in os.listdir(export_dir):
        ...
        if "Card" in filename:
            ...
            for e in entries:
                ...
                if content:
                    all_entries.append({"source": filename, "keys": keys, "content": content})
                    
        elif "Lorebook" in filename:
            ...
            for p in prompts:
                ...
                if content and "original" not in content and name != "Main Prompt" and name != "NSFW Prompt":
                    all_entries.append({"source": filename, "keys": [], "content": f"[{name}]\n{content}", "always_on": True})
    ...
    if match:
        ...
        out_path = os.path.join(export_dir, f"{world_name}_JanitorAI_Script.js")
        with open(out_path, 'w', encoding='utf-8') as f:
            f.write(final_content)
        print(f"Successfully wrote {len(all_entries)} entries to {out_path}")
    else:
